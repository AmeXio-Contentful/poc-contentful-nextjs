...
import {
  FooterComponent,
  FooterProps,
} from '@src/components/features/decoupled-components/footer/footer';
...
export const Footer = (props: FooterFieldsFragment) => {
  ...
  if (footerContent && footerContent.menuItemsCollection) {
    doormatProps.items = footerContent.menuItemsCollection.items.map(collection => {
      const doormatItem: DoormatItem = {
        ...
        children:
          collection?.featuredPagesCollection?.items.map(collectionChild => {
  ...
  footerProps.legalLinks =
    footerContent?.legalLinks?.featuredPagesCollection?.items?.map(item => ({
