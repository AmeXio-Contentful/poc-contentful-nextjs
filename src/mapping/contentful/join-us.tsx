...
export const JoinUsComponent = (props: DefaultQueryParameters) => {
  ...
  const joinUsItems: JoinUsItem[] | undefined = componentData?.teasersCollection?.items.map(
    item => ({
      title: item?.title || '',
      image: item?.image || '',
      id: item?.sys.id,
  ...
  const joinUsClients: JoinUsClient[] | undefined = componentData?.clientsCollection?.items.map(
    item => ({
      title: item?.title || '',
      url: item?.url || '',
      id: item?.sys.id,
