...
export const ReferenceClientsComponent = (props: DefaultQueryParameters) => {
  ...
  const componentData: ComponentRerenceClientFragment | null | undefined =
    data.data?.referenceClients;
  // adjust data over here to custom component typing
  const clients: ReferenceClient[] | null | undefined =
    componentData?.clientsCollection?.items?.map(item => ({
      id: item?.sys.id || '',
      image: item?.image || '',
      title: item?.title || '',
