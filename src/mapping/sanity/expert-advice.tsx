...
export function ExpertAdvice(props: any) {
  const { buttonText, buttonUrl, title, subtitle, expertAdvices } = props.componentProps;
  const buttonProps = {
    buttonText,
    buttonUrl,
    ...
    >
      <div className="w-4/5 ml-auto text-left">
        ...
        <div className="flex self-stretch flex-col gap-y-12 mt-4 mb-8">
          {
            expertAdvices && expertAdvices.map((item) => (
              <div key={item.advice}>
                <a href={item.url}>
                  <button
